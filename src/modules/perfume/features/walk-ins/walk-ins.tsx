...
import { useGetTodaysWalkInsQuery, useCheckOutWalkInMutation, useConfirmWalkInPaymentMutation, useLogDishSelectionMutation, useCreateWalkInMutation } from '../../../../redux/api/perfume-store/walk-ins.api';
...
export const WalkIns: React.FC = () => {
  ...
  const { data: response, isLoading } = useGetTodaysWalkInsQuery(user?.privateStoreId || '', {
  ...
  const handleAddNew = async (data: any) => {
    if (!user?.privateStoreId) return;
    try {
      await createWalkIn({ 
        storeId: user.privateStoreId,
    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto min-h-screen">
      ...
      <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage today's store walk-ins and guest passes</p>
        </div>
        <button
