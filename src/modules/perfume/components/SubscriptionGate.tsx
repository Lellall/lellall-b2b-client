...
const SubscriptionGate: React.FC<SubscriptionGateProps> = ({ storeId, children }) => {
  const { user } = useSelector(selectAuth);
  const { formatCurrency } = useCurrency();
  const { data: status, isLoading } = useGetPerfumeBillingStatusQuery(storeId, {
    skip: !storeId,
  ...
  if (!storeId || isLoading || !status) return <>{children}</>;
  ...
  return (
    <Overlay>
      <Card>
        ...
        <Subtitle>
          Your free trial has ended. Subscribe to continue using the POS, inventory, and dashboard for your store.
        </Subtitle>
        <AmountRow>
