...
const Subscriptions = () => {
    ...
    const [selectedProvider, setSelectedProvider] = useState<
        "flutterwave" | "paystack" | null
    >(null); // Track selected provider
    ...
    const planStyles = {
        "Basic Plan": {
            background: "#1E2A38", // Deep navy blue-gray
            color: "#A9CCE3", // Soft sky blue
        },
        "Standard Plan": {
            background: "#2E4057", // Charcoal blue
            color: "#D4A017", // Mustard gold
        },
        "Business Plan": {
            background: "#1ABC9C", // Vibrant teal
            color: "#ECF0F1", // Light gray
        },
        "Premium Plan": {
            background: "linear-gradient(to right, #D4A017, #8E44AD)", // Gold to purple
    ...
    const handleInitiatePayment = async (planId: string, planPrice: number) => {
        ...
        const paymentDto = {
            ...
            ...(selectedProvider === "flutterwave"
                ? {
                    tx_ref: reference,
                    redirect_url: `${window.location.origin}/verify-payment?provider=flutterwave&trxref=${reference}`,
                }
                : {
                    reference: reference,
                    callback_url: `${window.location.origin}/verify-payment?provider=paystack&reference=${reference}`,
                }),
        };
    ...
    if (isLoading) {
        return (
            <Cover>
                <div className="mt-20 text-center text-[${theme.colors.accent}]">
        ...
        return (
            <Cover>
                <div className="mt-20 text-center text-red-500">
                    Error loading plans: {JSON.stringify(error)}
                </div>
            </Cover>
        );
    }
                ...
                {filteredPlans.map((plan) => (
                    ...
                    >
                        <PricingCard
                            ...
                            background={
                                planStyles[plan.name]?.background || theme.colors.secondary
                            }
