...
import { useGetLoungeBillingStatusQuery } from '@/redux/api/private-lounge/billing.api';
...
const LoungeLayout: React.FC = () => {
  ...
  const { data: billingStatus } = useGetLoungeBillingStatusQuery(loungeId, { skip: !loungeId });
  const subStatus = billingStatus?.status || 'TRIAL';
  const daysLeft = billingStatus?.daysRemaining ?? 0;
  const planName = billingStatus?.paystackPlanCode ? 'Bespoke' : 'No Plan';
    ...
    <ThemeProvider theme={theme}>
      <LayoutWrapper>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          <Content>
            <SubscriptionGate loungeId={loungeId}>
              <Outlet />
            </SubscriptionGate>
          </Content>
        </div>
