...
export const LoungeStaffPage: React.FC = () => {
  ...
  const { data: staffList = [], isLoading, refetch } = useGetLoungeStaffQuery(loungeId, {
    skip: !loungeId,
  });
  ...
  const handleCreateStaff = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!loungeId) {
      toast.error('No private lounge assigned to this user.');
      return;
    }
    try {
      await createStaff({
      ...
      <div className="mb-8 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            Lounge Staff & Hostesses
          </h1>
          <p className="text-sm text-gray-500 mt-1">
            Manage operational team members, hostesses, and managers for your Private Lounge.
          </p>
        </div>
        <button
              ...
              <div className="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">
                    Create an account linked to your Private Lounge
                  </p>
                </div>
                <button
                  onClick={() => setShowModal(false)}
...
export default LoungeStaffPage;
